...
int main()
{
    ...
    int client, server; //1. client/server are two file descriptors These two variables store the values returned by the socket system call and the accept system call
    int portNum = 1500; //2. portNum is for storing port number on which the accepts connections
    bool isExit = false; // 3. isExit is bool variable which will be used to end the loop
    int bufsize = 1024; // 4. The server reads characters from the socket connection into a dynamic variable (buffer).
    char buffer[bufsize];

    struct sockaddr_in server_addr; //5. A sockaddr_in is a structure containing an internet address. This structure is already defined in netinet/in.h, so we don't need to declare it again.
 /*
    DEFINITION:
